...
def test_client_server():
    ...
    try:
        ...
        try:
            ...
            assert (
                result == "test/users/v1"
            ), f"Expected 'test/users/v1', got '{result}'"
        ...
        try:
            ...
            assert len(loaded) == len(
                df
            ), f"Row count mismatch: {len(loaded)} vs {len(df)}"
        ...
        print("\n--- Test 5: Load with Filter ---")
        try:
            filtered = client.load("test/users/v1", filter="age > 30 AND active = true")
        ...
        try:
            subset = client.load("test/users/v1", columns=["name", "score"], limit=10)
        ...
        try:
            ...
            assert (
                result == "test/users/v2"
            ), f"Expected 'test/users/v2', got '{result}'"
